...
//Note:here, we only care each DRAM die's bus width, no care about how many die used
...
//GRF_SOC_CON2
#define ddrpubl_lpddr3_en(ch,n)      (((n)<<(10+(3*(ch)))) | ((1<<(10+(3*(ch))))<<16))
#define ddrupctl_lpddr3_odt_en(ch,n) (((n)<<(9+(3*(ch)))) | ((1<<(9+(3*(ch))))<<16))
#define ddrupctl_bst_disable(ch,n)   (((n)<<(8+(3*(ch)))) | ((1<<(8+(3*(ch))))<<16))


#define DDR_SET_16BIT(ch, n)          do{g_grfReg->GRF_SOC_CON[0] = ddr_16bit_en(ch,n);}while(0)
#define DDR_SET_DDR3_TYPE(ch,n)       do{g_grfReg->GRF_SOC_CON[0]=msch_mainddr3(ch,n);}while(0)
#define DDR_SET_LPDDR3_TYPE(ch,n)     do{g_grfReg->GRF_SOC_CON[2]=ddrpubl_lpddr3_en(ch,n)|ddrupctl_bst_disable(ch,n)|ddrupctl_lpddr3_odt_en(ch,n);}while(0)
#define DDR_SET_LPDDR3_ODT(ch,n)      do{g_grfReg->GRF_SOC_CON[2]=ddrupctl_lpddr3_odt_en(ch,n);}while(0)
#define DDR_STRIDE(n)                 do{g_sgrfReg->SGRF_SOC_CON[2]=(0x1F<<16)|(n);}while(0)
