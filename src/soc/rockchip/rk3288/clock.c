...
/***********************************************************/
#define CRU_GET_REG_BITS_VAL(reg, bits_shift, msk) \
	(((reg) >> (bits_shift))&(msk))
#define CRU_W_MSK(bits_shift, msk) \
	((msk) << ((bits_shift) + 16))
#define CRU_SET_BITS(val, bits_shift, msk) \
	(((val)&(msk)) << (bits_shift))
...
#define PLL_CLKOD(val) \
	PLL_CLKFACTOR_SET(val, PLL_OD_SHIFT, PLL_OD_MSK)
#define PLL_NO(reg) \
	PLL_CLKFACTOR_GET(reg, PLL_OD_SHIFT, PLL_OD_MSK)
#define PLL_NO_SHIFT(reg) \
	PLL_CLKFACTOR_GET(reg, PLL_OD_SHIFT, PLL_OD_MSK)
#define PLL_CLKOD_SET(val) \
	(PLL_CLKOD(val) | CRU_W_MSK(PLL_OD_SHIFT, PLL_OD_MSK))

#define PLL_NR_MSK	(0x3F)
#define PLL_NR_SHIFT	(8)
#define PLL_CLKR(val) \
	PLL_CLKFACTOR_SET(val, PLL_NR_SHIFT, PLL_NR_MSK)
#define PLL_NR(reg) \
	PLL_CLKFACTOR_GET(reg, PLL_NR_SHIFT, PLL_NR_MSK)
#define PLL_CLKR_SET(val) \
	(PLL_CLKR(val) | CRU_W_MSK(PLL_NR_SHIFT, PLL_NR_MSK))
...
/*******************MODE BITS***************************/
/* npll mode bit: bit14-bit15 */
#define PLL_MODE_MSK(id) \
	((id == NPLL_ID) ? (0x3 << 14) : (0x3 << ((id) * 4)))
#define PLL_MODE_SLOW(id) \
	((id == NPLL_ID) ? \
	(0x0<<14)|(0x3<<(16+14)) : ((0x0<<((id)*4))|(0x3<<(16+(id)*4))))
#define PLL_MODE_NORM(id) \
	((id == NPLL_ID) ? \
	(0x1<<14)|(0x3<<(16+14)) : ((0x1<<((id)*4))|(0x3<<(16+(id)*4))))
#define PLL_MODE_DEEP(id) \
	((id == NPLL_ID) ?	\
	(0x2<<14)|(0x3<<(16+14)) : ((0x2<<((id)*4))|(0x3<<(16+(id)*4))))
...
#define cru_readl(offset) \
	readl((const void *)(CRU_BASE_ADDR + offset))
#define cru_writel(v, offset) \
	do { \
		writel(v, (int *)(CRU_BASE_ADDR + offset)); \
	} while (0)


#define _APLL_SET_CLKS( \
		khz, nr, nf, no, \
		_a12_div, \
		_mp_div, \
		_m0_div, \
...
static void rkclk_pll_wait_lock(enum rk_plls_id pll_id)
{
	...
	while (1) {
		if (readl((const void *) \
			(REG_FILE_BASE_ADDR + GRF_SOC_STATUS1)) \
			& bit)
			break;
		clk_loop_delayus(1);
	}
...
static int rkclk_pll_clk_set_rate(enum rk_plls_id pll_id, \
		uint32_t mHz, pll_callback_f cb_f)
{
 ...
 */
static void rkclk_periph_ahpclk_set( \
		uint32_t pll_src, \
		uint32_t aclk_div, \
		uint32_t hclk_div, \
		uint32_t pclk_div)
 ...
 * when call this function, make sure pll is in slow mode
 */
static void rkclk_cpu_coreclk_set( \
		uint32_t pll_src, \
		uint32_t a12_core_div, \
		uint32_t aclk_core_mp_div, \
		uint32_t aclk_core_m0_div)
{
 ...
 * when call this function, make sure pll is in slow mode
 */
static void rkclk_cpu_l2dbgatclk_set( \
		uint32_t l2ram_div, \
		uint32_t atclk_core_div, \
		uint32_t pclk_core_dbg_div)
{
...
{
	rkclk_cpu_coreclk_set(CPU_SRC_ARM_PLL, \
		clkset->a12_core_div, \
		clkset->aclk_core_mp_div, \
		clkset->aclk_core_m0_div);
	rkclk_cpu_l2dbgatclk_set(clkset->l2ram_div, \
		clkset->atclk_core_div, \
		clkset->pclk_dbg_div);
}
...
{
	rkclk_periph_ahpclk_set(PERIPH_SRC_GENERAL_PLL, \
		clkset->aclk_div, \
		clkset->hclk_div, \
		clkset->pclk_div);
}
