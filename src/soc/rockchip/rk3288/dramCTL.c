...
void set_bw(unsigned int n)
{
	...
	if (n == 1) {
		...
		pPHY_Reg->DATX8[2].DXGCR |= 1;
		pPHY_Reg->DATX8[3].DXGCR |= 1;
		/* freq > DLL bypass freq */
		if (pDDR_Reg->TOGCNT1U > 250) {
			/*enable DLL*/
			pPHY_Reg->DATX8[2].DXDLLCR &= ~0x80000000;
			pPHY_Reg->DATX8[3].DXDLLCR &= ~0x80000000;
		}
...
static void pctl_cfg(void)
{
	...
	ddr_copy((unsigned int *)&(pDDR_Reg->TOGCNT1U),
		(unsigned int *)&(ddr_timing.pctl_timing.togcnt1u), 34);
	...
	case DDR3:
		...
		pDDR_Reg->TRSTH = 500;
		pDDR_Reg->MCFG = mddr_lpddr2_clk_stop_idle(0) | ddr3_en
			| bl_ddr2_ddr3 | tfaw_cfg(5) | pd_exit_slow
			| pd_type(1) | pd_idle(0);
		bDDR3 = 1;
		break;
	}
...
static void phy_cfg(void)
{
	...
	/*DDR PHY Timing*/
	ddr_copy((unsigned int *)&(pPHY_Reg->DTPR[0]),
		(unsigned int *)&(ddr_timing.phy_timing.dtpr0), 7);
	pMSCH_Reg->ddrtiming.d32 = (pMSCH_Reg->ddrtiming.b.BwRatio)
		| ddr_timing.noc_timing.d32;
	...
	pPHY_Reg->PTR[1] = tDINIT1(213) | tDINIT0(266525);
	/* tDINIT3=1us (533MHz), tDINIT2=200us (533MHz) */
	pPHY_Reg->PTR[2] = tDINIT3(534) | tDINIT2(106610);

	odt = 0;
	switch (dramType) {
	case DDR3:
		pPHY_Reg->PGCR = (pPHY_Reg->PGCR & (~0x1F)) | DFTLMT(0) |
			DFTCMP(0) | DQSCFG(0) | ITMDMD(0);
		pPHY_Reg->DCR.b.DDRMD = 3;
...
void phy_init(void)
{
	...
	for (i = 0; i < 10; i++)
		;
	while ((pPHY_Reg->PGSR & (IDONE | DLDONE | ZCDONE)) !=
		(IDONE | DLDONE | ZCDONE))
		;
}
...
void memory_init(void)
{
	...
	if (dramType == DDR3)
		pPHY_Reg->PIR |= INIT | DRAMRST | DRAMINIT | LOCKBYP
			| ZCALBYP | CLRSR | ICPC;
	else
		pPHY_Reg->PIR |= INIT | DRAMINIT | LOCKBYP | ZCALBYP
			| CLRSR | ICPC;
	for (i = 0; i < 10; i++)
		;
	...
	while (1) {
		...
		case Low_power:
			...
			while ((pDDR_Reg->STAT.b.ctl_stat) != Access)
				;
			/* wait DLL lock */
			while ((pPHY_Reg->PGSR & DLDONE) != DLDONE)
				;
		case Access:
		case Init_mem:
...
void move_to_Access_state(void)
{
	...
	while (1) {
		value = pDDR_Reg->STAT.b.ctl_stat;
		if ((value == Access) || ((pDDR_Reg->STAT.b.lp_trig == 1)
...
retry:
	...
	set_cs(pChInfo->cs | 1);
	/*DS=40ohm,ODT=155ohm*/
	pPHY_Reg->ZQ1CR[0] = (0x1<<28) | (0x2<<15) | (0x2<<10)
		| (0x19<<5) | 0x19;
	/*DS=40ohm,ODT=155ohm*/
	pPHY_Reg->ZQ0CR[0] = (0x1<<28) | (0x2<<15) | (0x2<<10)
		| (0x19<<5) | 0x19;
