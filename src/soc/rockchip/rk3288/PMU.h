...
#define MaskRegBits32(addr, y, z)           writel((readl(addr)&~(y))|(z),addr)

//ch=1:one channel
//ch=2:two channel
#define RECORD_CH_CNT(n)   MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), (0x1<<12), (((n)-1)<<12))
//ch=1:channel a valid
//ch=2:channel b valid
//ch=3:channel a&b valid
#define RECORD_CH_INFO(ch)  MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), (0x3<<28), ((ch)<<28))
//row_3_4:0=normal, 1=6Gb or 12Gb
#define RECORD_CH_ROW_INFO(ch, row_3_4) MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), (0x1<<(30+(ch))), ((row_3_4)<<(30+(ch))))

#define RECORD_DRAMTYPE_INFO(dramType)  MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), (0x7<<13), ((dramType)<<13))
/* all ch below, 
   ch=0, channel a; 
   ch=1, channel b;
 */
#define RECORD_CS_INFO(ch,cs)   MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), (0x1<<(11+(ch)*16)), (((cs)-1)<<(11+(ch)*16)))
#define RECORD_COL_INFO(ch,col) MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), (0x3<<(9+(ch)*16)), (((col)-9)<<(9+(ch)*16)))
#define RECORD_BK_INFO(ch,bk)   MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), (0x1<<(8+(ch)*16)), ((((bk)==3) ? 0:1)<<(8+(ch)*16)))
#define RECORD_CS0_ROW_INFO(ch,cs0_row) MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), (0x3<<(6+(ch)*16)), (((cs0_row)-13)<<(6+(ch)*16)))
#define RECORD_CS1_ROW_INFO(ch,cs1_row) MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), (0x3<<(4+(ch)*16)), (((cs1_row)-13)<<(4+(ch)*16)))
#define RECORD_BW_INFO(ch,bw)   MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), (0x3<<(2+(ch)*16)), ((2>>(bw))<<(2+ch*16)))
#define RECORD_DIE_BW_INFO(ch,die_bw)   MaskRegBits32(&(pPMU_Reg->PMU_PMU_SYS_REG[2]), ((0x3)<<((ch)*16)), ((2>>(die_bw))<<((ch)*16)))
...
#define READ_CS0_ROW_INFO(ch)  (13+(((pPMU_Reg->PMU_PMU_SYS_REG[2])>>(6+(ch)*16))&0x3))
#define READ_CS1_ROW_INFO(ch)  (13+(((pPMU_Reg->PMU_PMU_SYS_REG[2])>>(4+(ch)*16))&0x3))        
