...
/* used for ddr3 only */
static __sramdata unsigned int ddr_speed_bin;
/* one chip cs capability */
static __sramdata unsigned int ddr_capability_per_die;
...
static unsigned int __sramlocalfunc ddr_get_parameter(unsigned int nMHz)
{
	...
	#if (CHANNEL_DRAM_TYPE == DDR3)
	{
		...
		p_publ_timing->mr[2] = DDR3_MR2_CWL(cwl);
		...
		/*
		 * tRFC, 90ns(512Mb),110ns(1Gb),160ns(2Gb),
		 * 300ns(4Gb),350ns(8Gb)
		 */
		...
		p_noc_timing->b.WrToMiss = (cwl+((DDR3_tWR*nMHz+999)/1000)
			+cl+cl);
		/* tRC=tRAS+tRP */
		p_pctl_timing->trc = ((((ddr3_tRC_tFAW[ddr_speed_bin]>>8)*nMHz
			+999)/1000)&0x3F);
		p_noc_timing->b.ActToAct = (((ddr3_tRC_tFAW[ddr_speed_bin]>>8)
			*nMHz+999)/1000);
		p_publ_timing->dtpr0.b.tRC = (((ddr3_tRC_tFAW[ddr_speed_bin]>>8)
			*nMHz+999)/1000);

		p_pctl_timing->trtw = (cl+2-cwl);
		...
		p_pctl_timing->tras = (((DDR3_tRAS*nMHz+(nMHz>>1)+999)/1000)
			&0x3F);
		p_publ_timing->dtpr0.b.tRAS = ((DDR3_tRAS*nMHz+(nMHz>>1)+999)
			/1000);
		/* tRCD=CL */
		p_pctl_timing->trcd = cl;
		p_publ_timing->dtpr0.b.tRCD = cl;
		/*
		 * tRRD = max(4nCK, 7.5ns), DDR3-1066(1K), DDR3-1333(2K)
		 *        , DDR3-1600(2K)
		 ...
		 *         1:ODT may not be turned on until one
		 *           clock after the read post-amble
		 */
